...
B                   Crouch
X                   Use item
Y (tap)             Holster weapon
Y (hold)            Drop item

LT                  Block
RT                  Attack / use weapon
RT (hold + slow release, with arrow equipped)
                    Unequip arrow

LB + Left Stick     Lean left / right
RB (hold)           Radial inventory menu (select with left stick)

LS click            Toggle slow/fast movement mode
RS click            Toggle creep

D-pad Up            Select broadhead arrow
D-pad Down (tap)    Select sword
D-pad Down (hold)   Select blackjack
D-pad Left          Cycle item previous
D-pad Right         Cycle item next
